...
class BaseZeroCurve(object):
    ...
    def __init__(self, maturities, rates, interp_method):
        ...
        self._forward_end = maturities[1:]
    ...
    def fwd_rate_simple(self, expiry: float, tenor: float):
        return (self.df(expiry) / self.df(expiry + tenor) - 1) / tenor

    def fwd_rate_year(self, expiry: float, tenor: float):
        return (self.df(expiry) / self.df(expiry + tenor))**(1/tenor)-1
    ...
    def interpolate(self, expiry: List):
        ...
        if self._interp_method[1] == 'R':
            ...
            return self.result(expiry) / expiry
        else:
            interp_rates = []
            for date in expiry:
